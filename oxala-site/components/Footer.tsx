...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-[#e2c28a]/70 bg-gradient-to-b from-[#fff3df] via-[#f2cf96] to-[#cc8c3d] text-brand-ink">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 text-sm text-brand-ink/80 sm:flex-row sm:items-start sm:justify-between sm:px-6 lg:px-8">
            ...
            <div className="flex gap-3">
              {socialLinks.map((social) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 rounded-full border border-brand-line/60 bg-white/70 px-3 py-1 text-xs font-semibold text-brand-ink/80 transition hover:border-brand-gold hover:bg-white hover:text-brand-gold"
                >
                  {social.icon}
                  {social.label}
                </a>
